...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Works for all pages: main, about, dates
  const header = 
    document.getElementById('mainPageHeader') ||
    document.getElementById('aboutPageHeader') ||
    document.getElementById('datesPageHeader');

  // ---------------------------
  // HELP & SERVICE BUTTON LOGIC
  ...
  function showClose() {
    ...
    hamburger.innerHTML = `
      <span class="header-font font-thin text-black text-sm sm:text-base lg:text-xl transition-colors duration-200">✕</span>
    `;
  ...
  if (hamburgerContainer) {
    hamburgerContainer.addEventListener('mouseenter', () => {
      hamburger.querySelectorAll('span').forEach(span => {
        if (span.classList.contains('bg-black')) span.classList.add('bg-red-500');
        else span.classList.add('text-red-500');
      });
  ...
  function toggleDropdown() {
    ...
    if (isClosed) {
      ...
      const maxHeight = 
        isMobile 
          ? window.innerHeight 
          : Math.min(dropdownMenu.scrollHeight, window.innerHeight * 0.9);
